...
class MatrixTreeGenerator:
    ...
    def generate_matrix(self):
        ...
        """
        n = 2**(2*self.k)       # {0,1}^n, with n=2^(2k) (nodes on last level)
        m = 2**(2*self.k)-1     # {0,1}^m, with m=2^(2k)-1 (all nodes until level height-1)
        inputs = self.zero_one_subsets(n)    
        algorithms = self.zero_one_subsets(m)
    ...
    def compute_cell_payoff(self, A, I)-> int:
        ...
        """        
        tree = BinaryNorTree(2*self.k, A, I)  # create (A,I) tree of height 2k
        payoff = tree.evaluate()            # evaluate tree
        return payoff
    
    def zero_one_subsets(self, n, limit=None):        
        """
